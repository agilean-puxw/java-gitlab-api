...
public class GitlabAPI {
    ...
    public List<GitlabCommit> getCommits(Serializable projectId, LocalDateTime since, LocalDateTime until, String branch) throws IOException {
        final Query query = new Query();
        if (branch != null) {
            query.append("ref_name", branch);
        }
        if (since != null) {
            query.append("since", since.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
        }
        if (until != null) {
            query.append("until", until.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
        }
        query.append("with_stats", "true");
